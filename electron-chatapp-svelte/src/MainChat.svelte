<script>
    import {msgs, groups, token, users, addUser, sortGroups, addUsersOfGroup, sortMsgs, fetch_groups, fetch_msgs, send_message, server_update, current_group } from './store.js'
...
<div class="flex h-screen text-gray-800 antialiased">
    <div class="flex h-full w-full flex-row overflow-x-hidden">
        ...
        <MsgsHolder activeGroup={activeGroup} bind:clearForm={clearMsgsForm} bind:scrollToBottom={scrollToBott} msgs={$msgs} sendMessage={sendMessage} users={$users} isActive={activeGroup == null ? false : true}></MsgsHolder>
    </div>
</div>
